...
# * create_async_engine:
#   - PostgresSQL과 연결을 만들기 위한 도구 (비동기 방식)
#   - DB 주소를 넣어서 '엔진'이라는 연결 도구를 만든다.
# * AsyncSession :
#   - DB에 연결한 후 데이터르 조회하거나 저장할 때 사용하는 세션
#   - 비동기 방식으로 작동하는 세션이다.
from sqlalchemy.ext.asyncio import create_async_engine, AsyncSession

# * sessionmaker :
# - 위에서 만든 엔진을 이용해서 실제 작업을 할 수 있는 '세션'을 만든다.
...
# ----------------------------------
# [1] PostgreSQL에 연결할 주소 설정(DB접속 정보)
# 형식 : postgresql+asyncpg://사용자:비밀번호@호스트/데이터베이스이름
# - postgresql+asuncpg : 비동기 방식으로 PostgreSQL에 접속할 수 있게 해주는 형식
# - todo_user : DB 사용자 이름
# - 1234 : 사용자 비밀번호
...
# - autocommit=Fasle : 자동 저장하지 않음 (commit()을 직접 호출해야 저장됨)
...
# - 이후에 만들 모든 테이블 클래스들을 이 Base를 상속받아 정의한다. 
...
# - 예 : 함수 정의해서 db=Depends(get_db)라고 쓰면 FastAPI가 알아서 실행해준다.
