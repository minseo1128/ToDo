...
# --------------------------------------------
# [ import 구문]
# 비동기 DB 세션과 Task 모델 Task 스키미를 불러온다.
# --------------------------------------------

# * AsyncSession:
#   - SQAlchemy에서 DB 작업을 비동기(async) 방식으로 수행할 수 있게 해주는 세션 객체
#   - FaskAPI에서 async def 함수와 함께 사용하며, commit.refresh 같은 작업 앞에는 await를 붙여야 한다.
from sqlalchemy.ext.asyncio import AsyncSession

# * task_model:
#   - 실제 DB에 저장될 Task 테이블 클래스가 정의되어 있음
# * task_schema:
#   - 사용자의 요청/응답 데이터를 저의한 Pydantic 스키미 클래스
...
async def create_task(db: AsyncSession,task_create: task_schema.TaskCreate) -> task_model.Task:
    ...
    # * commit 이후 Db기 지동 생성한 id 값을 포함한 최선 데이터를 다시 불러옴
    #   - 예 : task.id가 None이었다면, refresh 이후 실제 숫자가 들어오게 됨
    await db.refresh(task)
    
    # * 최종적으로 저장된 Task 객체를 변환 (API 응답에서 사용됨)
    return task 
...
async def delete_task(db:AsyncSession,original:task_model.Task) -> None:
    # * db.delete(original):
    #   - DB 세션에서 해당 Task객체를 삭제 대상으로 표시함
