...
# router는 여러기능(API 주소들)을 모아서 저장해두는 상자(모음집) 같은 역할을 한다.
router=APIRouter()

# -----------------------------
# 할 일 목록 보기 기능 (GET 방식)
# 예: /task 주소로 접속하면 전체 할 일 목록을 보여준다.
# -----------------------------
@router.get("/tasks",response_model=list[task_schema.Task])
# response_model -> 응답의 데이터 모양을 정해주는 옵션
# 여기서는 여러개의 Task 모델을 리스트 형태로 응답함
...
# ------------------------------
@router.get("/tasks", response_model=task_schema.TaskCreateResponse)
# task_body: 클라이언트가 보낸 할 일 데이터(title만 있음)
# TaskCreateResponse: 저장된 결과를 id를 포함해 응답함
async def list_tasks():
    return [task_schema.Task(id=1, title="첫 번째 ToDo 작업",done =False)]
    # 실제 DB가 없으므로, 예시 데이터를 직접 만들어 응답으로 보낸다.
    # task_schema.Task(...) 형태로 모델에 맞춰 값을 채운다.
...
# task_body : 클라이언트가 보낸 할 일 데이터 (title만 있음)
# TaskCreateResponse: 저장된 임시 결과로 id를 포함해 응답
...
# ------------------------------
# 할 일 삭제 기능 (DELETE 방식)
# 예: /tasks/3 -> 번호가 3인 할 일의 내용을 삭제한다. 
# ------------------------------
