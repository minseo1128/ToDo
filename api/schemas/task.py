...
# - ConfigDict: Pydantic v2부처 모델 설정(예: fromattributes)을 지정할 때 사용

# -----------------------------------------
# [1] 공통 속성 클래스: TaskBase
# - title(할 일 제목)만 포함되어 있음음
...
class TaskCreateResponse(TaskCreate):
    ...
    # SQLAlchemy 같은 ORM객체를 이 모델로 자동 변환할 수 있게 설정
    # * 기존 orm_mode = True 설정이 Pydantic v2에서는 from_attribute로 대체됨됨
        
# ------------------------------------------
# [4] 할 일 전체 정보를 표현하는 모델: Task
# - 목록 조회나 상세 보기 등에 사용됨
# - id(번호), title(제목), done(완료 여부) 포함함
# ------------------------------------------
class Task(TaskBase): 
    id:int  # 할 일 번호(정수, 예:1,2,3 등)

    done: bool=Field(
        default=False, # 기본값은 '완료되지 않음(Fasle)'
