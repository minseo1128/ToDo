...
async def async_client() -> AsyncGenerator[AsyncClient,None]:
    ...
    async_session=sessionmaker(
        autocommit=False,   # 명식적으로 commit 해야 저장됨
    ...
    # ------------------------
    # 3. get_db() 함수를 테스트용 DB와 연결되지 않도록 override
...
async def test_done_flag(async_client):
    ...
    # ------------------------
    # [2] 할 일을 완료 처리 (PUT 요청)
    # - 이 테스트에서는 PUT/tasks라는 주소에 요청을 보냄
    ...
    # ------------------------
    # [4] 완료 처리된 작업을 완료 해제 (DELETE 요청청)
    # - /tasks/1/done 주소로 요청을 보내면 완료 상태가 해제됨 (Fasle로 변경됨)
    # ------------------------
